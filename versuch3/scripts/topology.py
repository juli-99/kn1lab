...
class MyTopo(Topo):
    def __init__(self):
        ...
        burak = self.addHost('burak', ip='10.0.2.2/25')  # 2.2

        nas = self.addHost('nas', ip='10.0.1.2/29')

        # create switch
        sw1 = self.addSwitch('sw1')
        sw2 = self.addSwitch('sw2')  # 2.2

        # create router
        r1 = self.addHost('r1', ip='10.0.0.1/26')
        r2 = self.addHost('r2', ip='10.0.2.1/25')  # 2.2
        ...
        self.addLink(burak, sw2)  # 2.2

        # - nas over router to switch
        self.addLink(r1, sw1)
        self.addLink(nas, r1)
        self.addLink(r2, sw2)  # 2.2
        self.addLink(r2, r1)  # 2.2
...
def conf(network):
    ...
    network['r1'].cmd('ip addr add 10.0.1.1/29 dev r1-eth1')    # correct subnet to 10.0.1.1 from 10.0.2.1 (1.5)
    network['r1'].cmd('ip addr add 10.0.1.64/31 dev r1-eth2')   # 2.2
    network['r1'].cmd('sysctl net.ipv4.conf.all.forwarding=1')

    network['r2'].cmd('ip addr add 10.0.2.1/25 dev r2-eth0')    # 2.2
    network['r2'].cmd('ip addr add 10.0.1.65/31 dev r2-eth1')   # 2.2
    network['r2'].cmd('sysctl net.ipv4.conf.all.forwarding=1')  # 2.2
    ...
    network['burak'].cmd('ip route add default via 10.0.2.1')    # 2.2
    network['r1'].cmd('ip route add 10.0.2.0/25 via 10.0.1.65')  # 2.2
    network['r2'].cmd('ip route add 10.0.1.0/29 via 10.0.1.64')  # 2.2
